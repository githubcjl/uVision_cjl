				...
				for(int i = 0; i < m_nResSize; ++i){
					...
					switch(m_lpDevRes[i].iSUBSampleMode){
						case SUBSAMPLE_MODE_BIN:
							strText.Append(_T(" Bin"));
							break;
						case SUBSAMPLE_MODE_SKIP:
							strText.Append(_T(" Skip"));
							break;
						case SUBSAMPLE_MODE_SUM:
							strText.Append(_T(" Sum"));
							break;
					}
...
BEGIN_MESSAGE_MAP(CROIDlg, CDialog)
	ON_WM_HSCROLL()
	ON_BN_CLICKED(IDC_BTN_SETROI,	OnBnClickedSetROI)
	ON_BN_CLICKED(IDC_BTN_MAXIMIZE, OnBnClickedMaximize)
...
void CROIDlg::OnBnClickedSetROI()
{
	if(m_pDevice != NULL){
		ResolutionParam param;
...
void CROIDlg::OnBnClickedMaximize()
{
	CString strBuf;
	m_ctrlOffseX.SetPos(0);
